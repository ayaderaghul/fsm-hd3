...
(define OPPONENT-COMBINATION
  (list 'DD 'DH 'HD 'HH))
(define (random-opponent-combination)
  (list-ref OPPONENT-COMBINATION (random (length OPPONENT-COMBINATION))))
...
(define (hawk)
  ...
  (define body (hash 0 (state 'H (hash 'DD 0 'DH 0 'HD 0 'HH 0))))
  (automaton head body))

(define (dove)
  (define head (hash 'INITIAL 0 'CURRENT 0 'PAYOFF 0))
  (define body (hash 0 (state 'D (hash 'DD 0 'DH 0 'HD 0 'HH 0))))
  (automaton head body))
...
(define (add-state a)
  ...
  (define new-body
    (hash-union
     (hash-set body mutate-state
              (state action
                     (hash-set dispatch (random-opponent-combination) l)))
     (hash l (make-state))))
...
(define PAYOFF-TABLE
  (list
   (list
    (list (list 3 3 3) (list 1 1 5))
    (list (list 1 5 1) (list 1 0 0)))
   (list
    (list (list 5 1 1) (list 0 1 0))
    (list (list 0 0 1) (list 0 0 0)))))
...
(define (interact au1 au2 au3 rounds delta)
  (match-define (automaton head1 body1) au1)
  (match-define (automaton head2 body2) au2)
  (match-define (automaton head3 body3) au3)
  (define-values (next1 next2 next3 pay1 pay2 pay3 round-results)
    (for/fold ([current1 (hash-ref head1 'CURRENT)]
               [current2 (hash-ref head2 'CURRENT)]
               [current3 (hash-ref head3 'CURRENT)]
               [payoff1 (hash-ref head1 'PAYOFF)]
               [payoff2 (hash-ref head2 'PAYOFF)]
               [payoff3 (hash-ref head3 'PAYOFF)]
               [round-results '()])
              ([_ (in-range rounds)])
      #:final (> (random) delta)
      (match-define (state action1 dispatch1) (hash-ref body1 current1))
      (match-define (state action2 dispatch2) (hash-ref body2 current2))
      (match-define (state action3 dispatch3) (hash-ref body3 current3))

      (match-define (list pay1 pay2 pay3) (payoff action1 action2 action3))
      (define n1 (hash-ref dispatch1 (symbol-append action2 action3)))
      (define n2 (hash-ref dispatch2 (symbol-append action3 action1)))
      (define n3 (hash-ref dispatch3 (symbol-append action1 action2)))

      (define round-result (list pay1 pay2 pay3))
      (values n1 n2 n3
              (+ payoff1 (* pay1 (- 1 delta)))
              (+ payoff2 (* pay2 (- 1 delta)))
              (+ payoff3 (* pay3 (- 1 delta)))
               ;(+ payoff1 pay1)
               ;(+ payoff2 pay2)
  ...
  (values
   ...
   (automaton (hash-set head3 'PAYOFF (round5 pay3)) body3)))


(define (interact-d au1 au2 au3 rounds delta)
  (match-define (automaton head1 body1) au1)
  (match-define (automaton head2 body2) au2)
  (match-define (automaton head3 body3) au3)
  (define-values (next1 next2 next3 pay1 pay2 pay3 round-results)
...
(define (interact* au1 au2 au3 rounds delta)
  (with-handlers ([exn:fail?
                   (lambda (e) (values (list 'I-AM-HERE!!!) au1 au2 au3))])
    (interact-d au1 au2 au3 rounds delta)))

(define (interact-s s1 s2 s3 rounds delta)
  (define-values (result a1 a2 a3) (interact-d s1 s2 s3 rounds delta))
  (round2 (hash-ref (automaton-head a1) 'PAYOFF)))

(define (interact-2 s1 s2 s3 rounds delta)
  (define-values (result a1 a2 a3) (interact-d s1 s2 s3 rounds delta))
  (list
   (round2 (hash-ref (automaton-head a1) 'PAYOFF))
   (round2 (hash-ref (automaton-head a2) 'PAYOFF))
   (round2 (hash-ref (automaton-head a3) 'PAYOFF))))
