...
(define (convert x)
  (cond [(< (string-length x) 4) (string->number x)]
        [else
         ...
         (match-define (list o i) (string-split code ") ("))
         (define (string->au str)
           (define string->pieces (string-split str " "))
           (define pieces->numbers (map string->number string->pieces))
           (recover-automaton pieces->numbers))
         (cons (string->au o) (string->au i))]))
