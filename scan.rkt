   ...
   (list
    ...
    (for/list ([(key value) (in-hash body)])
      ...
      (list
       key
       (list
        (if (equal? action 'D) 0 1)
        (for/list ([(key value) (in-hash dispatch)])
          value)))))))

(define (flatten-machine m)
  ...
  (define (recover-body s a-body)
    (cond [(empty? a-body) '()]
          [else
           ...
           (match-define (list state-id label d? h?)
                         first-state)
           (append (list
                    state-id
                    (state
...
(define (recover-machine m)
  (match-define (list o i) m)
  (cons
   (recover-automaton o)
   (recover-automaton i)))
...
(define (scan-flatten population)
  ...
  (foldl
   (lambda (au h)
     (hash-update h (flatten-machine au) add1 0))
   (hash)
   p))
...
(define (scan-defects-tft population)
  (let ([ranking (scan (vector-map reset-machine population))]
        [d (defects)]
        [t (tit-for-tat)])
    (list
     (hash-ref* ranking d)
